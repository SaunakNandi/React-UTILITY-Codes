...
export function Folder({ handleInsertNode, explorer }) {
  const { tabId, setTabId, showInput, setShowInput } =
        ...
        <div>
          <div
            ...
            onClick={(e) => {
              e.stopPropagation();
              setExpand(!expand);
              setTabId(explorer.id);
            }}
            style={{
              backgroundColor: explorer.id === tabId ? "gray" : "lightgray",
            }}
          >
          ...
          <div
            style={{
              display:
                expand || (tabId == explorer.id && showInput.visible)
                  ? "block"
                  : "none",
              paddingLeft: 25,
            }}
