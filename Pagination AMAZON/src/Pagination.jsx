...
const Pagination = ({
  ...
  function generatePagination(start, end, visibleSlots, currentPage) {
    ...
    if (rightPointer >= end) rightPointer = end - 1;
    result = [start];
    if (leftPointer > start + 1) {
      result.push("...");
      // since "..." aleadry added visible slots is reduced and leftPointer is shifted 1 step right
      --visibleSlots;
      ++leftPointer;
    }
    if (rightPointer >= end - 1) {
      for (
        let i = leftPointer - (visibleSlots - (rightPointer - leftPointer + 1));
        i <= rightPointer;
        i++
      )
        result.push(i);
    } else {
      for (let i = leftPointer; i <= rightPointer - 1; i++) result.push(i);
      result.push("...");
    }
    result.push(end);
    return result;
  ...
  return (
    <div className="pagination">
      <button
        onClick={() => currentPage > start && onPageChange(currentPage - 1)}
      >
        ⬅️
      </button>
      {pagination.map((page) => (
      ...
      <button
        onClick={() => currentPage < end && onPageChange(currentPage + 1)}
      >
        ➡️
      </button>
    </div>
